# Bytom Node Sync Monitor

> Bytom node blocks sync monitor tools.
...
``` bash
# Bytom node sync monitor
* * * * * cd /data/monitor/exinpool/Bytom/sync && bash bytom_sync.sh >> bytom_sync.log &
```

The crontab will run every minute then you can check the log in the `bytom_sync.log`.

## Features

- Monitor Bytom node blocks sync
