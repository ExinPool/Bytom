...
``` bash
# Bytom node process monitor
* * * * * cd /data/monitor/exinpool/Bytom/process && bash bytom_process.sh >> bytom_process.log &
```

The crontab will run every minute then you can check the log in the `bytom_process.log`.
